...
# внутри пространства global. В нашем случае, это описывается командой create foo global. Далее мы объявляем внутри
...
def get(namespace, var):
    if var in myDict[namespace]['vars']:
        return namespace
    if myDict[namespace]['parent'] != None:
...
def create(namespace, var):
    myDict[namespace] = {"parent": var, 'vars': []}
    return


n = int(input())
for i in range(n):
    fun, namespace, var = input().split()
    if fun == "add":
        add(namespace, var)
    elif fun == "get":
